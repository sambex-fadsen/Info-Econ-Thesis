...
Since agent $b$ knows $a$'s posteriors, when $a$ announces their posteriors, this will not rule out anything from $b$'s current partition. Thus $b$ will just report their initial posteriors. 

Since $a$ knows $b$ knows $a$'s posteriors, $a$ knew their announcement would not rule out anything from $b$'s initial partition.

At this point, our scenario is equivalent to starting with $a$ hearing $b$'s initial posteriors. 

By the same reasoning as above, since $a$ knows $b$'s initial posteriors, $a$ will just report their initial posteriors.
